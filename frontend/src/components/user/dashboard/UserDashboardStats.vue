<template>
  <section class="grid gap-px overflow-hidden border border-gray-200 bg-gray-200 sm:grid-cols-2 xl:grid-cols-4 dark:border-dark-700 dark:bg-dark-700">
    <article v-if="!isSimple" class="bg-white p-4 sm:p-5 dark:bg-dark-900">
      <div class="flex items-center justify-between gap-3">
        <p class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('dashboard.balance') }}</p>
        <Icon name="creditCard" size="sm" class="text-primary-600 dark:text-primary-400" />
      </div>
      <p class="mt-3 font-mono text-2xl font-semibold tabular-nums text-gray-950 dark:text-white">${{ formatBalance(balance) }}</p>
      <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ t('common.available') }}</p>
    </article>

    <article class="bg-white p-4 sm:p-5 dark:bg-dark-900">
      <div class="flex items-center justify-between gap-3">
        <p class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('dashboard.todayCost') }}</p>
        <Icon name="dollar" size="sm" class="text-primary-600 dark:text-primary-400" />
      </div>
      <p class="mt-3 font-mono text-2xl font-semibold tabular-nums text-gray-950 dark:text-white" :title="t('dashboard.actual')">${{ formatCost(stats?.today_actual_cost || 0) }}</p>
      <p class="mt-1 truncate text-xs text-gray-500 dark:text-gray-400" :title="`${t('dashboard.standard')}: $${formatCost(stats?.today_cost || 0)}`">{{ t('common.total') }}: ${{ formatCost(stats?.total_actual_cost || 0) }}</p>
    </article>
    ...
    <article class="bg-white p-4 sm:p-5 dark:bg-dark-900">
      ...
      <p class="mt-3 font-mono text-2xl font-semibold tabular-nums text-gray-950 dark:text-white">{{ formatNumber(stats?.today_requests || 0) }}</p>
    ...
    <article class="bg-white p-4 sm:p-5 dark:bg-dark-900">
      <div class="flex items-center justify-between gap-3">
        ...
        <Icon name="key" size="sm" class="text-primary-600 dark:text-primary-400" />
      </div>
      <p class="mt-3 font-mono text-2xl font-semibold tabular-nums text-gray-950 dark:text-white">{{ stats?.total_api_keys || 0 }}</p>
  ...
  <section class="grid grid-cols-2 gap-px overflow-hidden border border-gray-200 bg-gray-200 lg:grid-cols-4 dark:border-dark-700 dark:bg-dark-700">
    <article class="min-w-0 bg-white p-4 dark:bg-dark-900">
      <p class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('dashboard.todayTokens') }}</p>
      <p class="mt-2 font-mono text-xl font-semibold tabular-nums text-gray-950 dark:text-white">{{ formatTokens(stats?.today_tokens || 0) }}</p>
      <p class="mt-1 truncate text-xs text-gray-500 dark:text-gray-400" :title="`${t('dashboard.input')}: ${formatTokens(stats?.today_input_tokens || 0)} / ${t('dashboard.output')}: ${formatTokens(stats?.today_output_tokens || 0)}`">{{ t('dashboard.input') }} {{ formatTokens(stats?.today_input_tokens || 0) }} · {{ t('dashboard.output') }} {{ formatTokens(stats?.today_output_tokens || 0) }}</p>
    </article>
    <article class="min-w-0 bg-white p-4 dark:bg-dark-900">
      <p class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('dashboard.totalTokens') }}</p>
      <p class="mt-2 font-mono text-xl font-semibold tabular-nums text-gray-950 dark:text-white">{{ formatTokens(stats?.total_tokens || 0) }}</p>
    ...
    <article class="min-w-0 bg-white p-4 dark:bg-dark-900">
      ...
      <div class="mt-2 flex items-baseline gap-2"><p class="font-mono text-xl font-semibold tabular-nums text-gray-950 dark:text-white">{{ formatTokens(stats?.rpm || 0) }}</p><span class="text-xs text-gray-500">RPM</span></div>
      <p class="mt-1 font-mono text-xs tabular-nums text-gray-500 dark:text-gray-400">{{ formatTokens(stats?.tpm || 0) }} TPM</p>
    </article>
    <article class="min-w-0 bg-white p-4 dark:bg-dark-900">
      <p class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ t('dashboard.avgResponse') }}</p>
      <p class="mt-2 font-mono text-xl font-semibold tabular-nums text-gray-950 dark:text-white">{{ formatDuration(stats?.average_duration_ms || 0) }}</p>
