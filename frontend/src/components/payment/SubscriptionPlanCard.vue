      ...
      <div class="mb-3 flex items-start justify-between gap-2">
        ...
        <div class="shrink-0 text-right">
          <div class="flex items-baseline gap-1">
            <span :class="['text-2xl font-extrabold tracking-tight', textClass]">{{ displayPrice || `$${plan.price}` }}</span>
          </div>
          <span class="text-[11px] text-gray-400 dark:text-dark-500">/ {{ validitySuffix }}</span>
          <div v-if="plan.original_price" class="mt-0.5 flex items-center justify-end gap-1.5">
            <span class="text-xs text-gray-400 line-through dark:text-dark-500">{{ originalDisplayPrice || `$${plan.original_price}` }}</span>
...
const props = defineProps<{ plan: SubscriptionPlan; activeSubscriptions?: UserSubscription[]; displayPrice?: string; originalDisplayPrice?: string }>()
