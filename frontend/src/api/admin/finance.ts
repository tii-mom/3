...
export async function setVoucherRiskLock(id: number, locked: boolean, reason = ''): Promise<Voucher> { return (await apiClient.post<Voucher>(`/admin/vouchers/${id}/risk-lock`, { locked, reason })).data }
...
export async function updateDistributionConfig(enabled: boolean, stack: boolean, totpCode: string): Promise<void> { await apiClient.put('/admin/distribution/config', { enabled, stack_with_legacy: stack, totp_code: totpCode }) }
