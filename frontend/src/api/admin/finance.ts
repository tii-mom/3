...
export interface DistributionPolicyInput { commission_freeze_hours: number; withdrawal_min_cny_minor: number; withdrawal_daily_limit: number; withdrawal_fee_bps: number; first_recharge_bonus_bps: number; first_recharge_bonus_cap_usd: string; tiers: DistributionConfig['tiers'] }
...
export async function setDistributionTierOverride(userId: number, tierOverride: number | null, reason: string): Promise<DistributionTierMember> { return (await apiClient.put<DistributionTierMember>(`/admin/distribution/members/${userId}/tier`, { tier_override: tierOverride, reason })).data }
