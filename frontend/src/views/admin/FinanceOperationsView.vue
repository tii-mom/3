    ...
    <div class="space-y-5">
      <header class="flex flex-col gap-4 border-b border-gray-200 pb-5 lg:flex-row lg:items-end lg:justify-between dark:border-dark-700">
        <div>
          ...
          <p class="mt-1 text-sm text-gray-500 dark:text-dark-400">{{ localText('推广返佣规则、推广成员、提现审核与返佣流水。返佣比例在商品上架时逐件设置，不再有档位。', 'Referral rebate rules, promoters, payout review, and the rebate ledger. Rebate rates are set per product at listing time — there are no tiers.') }}</p>
        </div>
      </header>
      ...
      <form class="space-y-4 border border-gray-200 bg-white/70 p-4 dark:border-dark-700 dark:bg-dark-900/60" @submit.prevent="publishPolicy">
        ...
        <p class="border border-gray-200 bg-gray-50 px-3 py-2 text-xs leading-5 text-gray-600 dark:border-dark-700 dark:bg-dark-800 dark:text-dark-300">{{ t('finance.admin.rebateSourceHint') }}</p>
      ...
      <section v-if="active === 'members'" class="space-y-4">
        <div class="flex flex-wrap items-center gap-3 border border-gray-200 p-4 dark:border-dark-700">
          <input v-model="memberSearch" class="input min-w-[260px] flex-1" :placeholder="t('finance.admin.memberSearchPlaceholder')" @keyup.enter="loadMembers" />
          <button class="btn btn-secondary btn-sm" :disabled="memberLoading" @click="loadMembers">{{ t('common.search') }}</button>
          <p class="w-full text-xs text-gray-500">{{ t('finance.admin.memberHint') }}</p>
        </div>
        <div class="overflow-x-auto border border-gray-200 dark:border-dark-700">
          <table class="w-full min-w-[860px] text-sm">
            <thead class="bg-gray-50 text-gray-500 dark:bg-dark-800">
              <tr>
                <th class="px-4 py-3 text-left">{{ t('finance.admin.user') }}</th>
                <th class="px-4 py-3 text-right">{{ t('finance.distribution.companyMembers') }}</th>
                <th class="px-4 py-3 text-right">{{ t('finance.distribution.teamVolume') }}</th>
                <th class="px-4 py-3 text-right">{{ t('finance.distribution.availableCommission') }}</th>
                <th class="px-4 py-3 text-right">{{ t('finance.distribution.frozenCommission') }}</th>
                <th class="px-4 py-3 text-right">{{ t('finance.distribution.lifetimeEarned') }}</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="item in members" :key="item.user_id" class="border-t border-gray-100 dark:border-dark-700">
                <td class="px-4 py-3"><span class="block font-medium text-gray-900 dark:text-white">{{ item.email }}</span><span v-if="item.username" class="text-xs text-gray-500">{{ item.username }} · #{{ item.user_id }}</span></td>
                <td class="px-4 py-3 text-right font-mono tabular-nums">{{ item.invitee_count }}</td>
                <td class="px-4 py-3 text-right font-mono tabular-nums">{{ cny(item.team_volume_cny_minor) }}</td>
                <td class="px-4 py-3 text-right font-mono tabular-nums text-emerald-600">{{ cny(item.available_cny_minor) }}</td>
                <td class="px-4 py-3 text-right font-mono tabular-nums">{{ cny(item.frozen_cny_minor) }}</td>
                <td class="px-4 py-3 text-right font-mono tabular-nums">{{ cny(item.lifetime_earned_cny_minor) }}</td>
              </tr>
              <tr v-if="members.length === 0"><td colspan="6" class="px-4 py-10 text-center text-gray-500">{{ t('common.noData') }}</td></tr>
            </tbody>
          </table>
        </div>
      </section>
      ...
      <section v-else-if="active === 'ledger'" class="overflow-x-auto border border-gray-200 dark:border-dark-700">
        <table class="w-full min-w-[980px] text-sm"><thead class="bg-gray-50 text-gray-500 dark:bg-dark-800"><tr><th class="px-4 py-3 text-left">ID</th><th class="px-4 py-3 text-left">{{ t('finance.distribution.order') }}</th><th class="px-4 py-3 text-left">User</th><th class="px-4 py-3 text-left">Beneficiary</th><th class="px-4 py-3 text-right">{{ t('finance.admin.level') }}</th><th class="px-4 py-3 text-right">{{ t('finance.distribution.rate') }}</th><th class="px-4 py-3 text-right">{{ t('finance.distribution.commission') }}</th><th class="px-4 py-3 text-left">{{ t('common.status') }}</th></tr></thead><tbody>
          <tr v-for="item in commissions" :key="(item.source ?? 'distribution') + '-' + item.id" class="border-t border-gray-100 dark:border-dark-700"><td class="px-4 py-3">#{{ item.id }}</td><td class="px-4 py-3">#{{ item.source_order_id }}</td><td class="px-4 py-3">#{{ item.source_user_id }}</td><td class="px-4 py-3">#{{ item.beneficiary_user_id }}</td><td class="px-4 py-3 text-right">L{{ item.depth }}</td><td class="px-4 py-3 text-right">{{ item.rate_bps / 100 }}%</td><td class="px-4 py-3 text-right font-medium text-emerald-600">{{ cny(item.amount_cny_minor) }}</td><td class="px-4 py-3">{{ item.status }}</td></tr>
        </tbody></table>
      </section>
...
import { createDistributionPolicyVersion, getDistributionConfig, getWithdrawalPayoutDetails, listAdminCommissions, listAdminVouchers, listAdminWithdrawals, listDistributionConversions, listDistributionMembers, listRechargeEvents, reverseRechargeEvent, setVoucherRiskLock, transitionWithdrawal, type AdminCommission, type AdminDistributionMember, type DistributionConversionAudit, type DistributionPolicyInput, type PayoutDetails, type RechargeEvent } from '@/api/admin/finance'
...
const tabs = ['members', 'withdrawals', 'vouchers', 'ledger'] as const
const active = ref<(typeof tabs)[number]>('members')
...
const members = ref<AdminDistributionMember[]>([])
const memberSearch = ref('')
const memberLoading = ref(false)
const payoutDetails = ref<PayoutDetails>()
const policyVersion = ref(1)
const publishingPolicy = ref(false)
const policy = reactive<DistributionPolicyInput>({ commission_freeze_hours: 168, withdrawal_min_cny_minor: 2000, withdrawal_daily_limit: 1, withdrawal_fee_bps: 0, first_recharge_bonus_bps: 1000, first_recharge_bonus_cap_usd: '10000' })
const purchaseMultiplier = ref('1')
const isZhLocale = computed(() => locale.value.startsWith('zh'))
const localText = (zh: string, en: string) => isZhLocale.value ? zh : en
type OperationKind = 'payment' | 'reject' | 'risk' | 'reverse'
...
async function load() {
  try {
    const [withdrawalPage, voucherPage, commissionPage, rechargePage, memberPage, distributionConfig] = await Promise.all([listAdminWithdrawals(), listAdminVouchers(), listAdminCommissions(), listRechargeEvents(), listDistributionMembers(), getDistributionConfig()])
...
async function loadMembers() { memberLoading.value = true; try { members.value = (await listDistributionMembers(memberSearch.value)).items } catch (error) { app.showError(extractApiErrorMessage(error)) } finally { memberLoading.value = false } }
function openOperation(kind: OperationKind, id: number, options: { locked?: boolean } = {}) {
...
async function publishPolicy() { if (!window.confirm(`${t('finance.admin.publishPolicy')}?\n${t('finance.admin.policyVersionHint')}`)) return; publishingPolicy.value = true; try { const result = await createDistributionPolicyVersion({ ...policy }); policyVersion.value = result.config_version; app.showSuccess(t('common.saved')); await load() } catch (error) { app.showError(extractApiErrorMessage(error)) } finally { publishingPolicy.value = false } }
