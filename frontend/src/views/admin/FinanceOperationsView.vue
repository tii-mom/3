    ...
    <div class="space-y-5">
      ...
      <form class="space-y-4 border border-gray-200 p-4 dark:border-dark-700" @submit.prevent="publishPolicy">
        ...
        <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-6">
          ...
          <label class="text-xs font-medium text-gray-500">{{ t('finance.admin.exchangeRate') }}<input v-model="exchangeRate" class="input mt-1 w-full" inputmode="decimal" /></label>
        </div>
        <div class="overflow-x-auto"><table class="w-full min-w-[1060px] text-sm"><thead class="bg-gray-50 text-gray-500 dark:bg-dark-800"><tr><th class="px-3 py-2 text-left">{{ t('finance.distribution.tier') }}</th><th class="px-3 py-2 text-right">{{ t('finance.distribution.threshold') }}</th><th v-for="(unit, unitIndex) in companyUnits" :key="unitIndex" class="px-3 py-2 text-right">{{ unit }} bps</th></tr></thead><tbody><tr v-for="tier in policy.tiers" :key="tier.tier" class="border-t border-gray-100 dark:border-dark-700"><td class="px-3 py-2 font-mono">{{ tierDisplay(tier.tier, tier.threshold_cny_minor) }}</td><td class="px-3 py-2"><input v-model.number="tier.threshold_cny_minor" class="input ml-auto w-32 text-right" type="number" :min="tier.tier === 0 ? 0 : 1" :aria-label="`${tierDisplay(tier.tier, tier.threshold_cny_minor)} ${t('finance.distribution.threshold')}`" /></td><td v-for="(_, rateIndex) in tier.rates_bps" :key="rateIndex" class="px-3 py-2"><input v-model.number="tier.rates_bps[rateIndex]" class="input ml-auto w-24 text-right" type="number" min="0" max="10000" :aria-label="`${companyUnits[rateIndex]} bps`" /></td></tr></tbody></table></div>
        <div class="flex flex-wrap items-center justify-between gap-3"><p class="text-xs text-gray-500">{{ t('finance.admin.exchangeRateHint', { rate: exchangeRate }) }}</p><div class="flex gap-2"><button type="button" class="btn btn-secondary" :disabled="savingRate" @click="saveExchangeRate">{{ savingRate ? t('common.saving') : t('finance.admin.saveExchangeRate') }}</button><button class="btn btn-primary" :disabled="publishingPolicy">{{ t('finance.admin.publishPolicy') }}</button></div></div>
      </form>
      ...
      <section v-else-if="active === 'conversions'" class="overflow-x-auto border border-gray-200 dark:border-dark-700">
        <table class="w-full min-w-[920px] text-sm"><thead class="bg-gray-50 text-gray-500 dark:bg-dark-800"><tr><th class="px-4 py-3 text-left">ID</th><th class="px-4 py-3 text-right">{{ t('finance.admin.user') }}</th><th class="px-4 py-3 text-right">{{ t('finance.distribution.cnyAmount') }}</th><th class="px-4 py-3 text-right">{{ t('finance.distribution.usdAmount') }}</th><th class="px-4 py-3 text-right">{{ t('finance.distribution.exchangeRate') }}</th><th class="px-4 py-3 text-right">{{ t('finance.admin.version') }}</th><th class="px-4 py-3 text-left">{{ t('finance.distribution.createdAt') }}</th></tr></thead><tbody><tr v-for="item in conversions" :key="item.id" class="border-t border-gray-100 dark:border-dark-700"><td class="px-4 py-3">#{{ item.id }}</td><td class="px-4 py-3 text-right">#{{ item.user_id }}</td><td class="px-4 py-3 text-right font-mono tabular-nums">{{ cny(item.amount_cny_minor) }}</td><td class="px-4 py-3 text-right font-mono tabular-nums">${{ item.usd_amount }}</td><td class="px-4 py-3 text-right font-mono tabular-nums">{{ item.usd_to_cny_rate }}</td><td class="px-4 py-3 text-right">v{{ item.config_version }}</td><td class="px-4 py-3">{{ date(item.created_at) }}</td></tr><tr v-if="conversions.length === 0"><td colspan="7" class="px-4 py-10 text-center text-gray-500">{{ t('common.noData') }}</td></tr></tbody></table>
      </section>
    </div>
  </AppLayout>
</template>
...
async function load() {
  try {
    ...
    exchangeRate.value = distributionConfig.usd_to_cny_rate || '7.15'
...
async function saveExchangeRate() { if (!security.totp) return app.showError('TOTP required'); savingRate.value = true; try { const result = await updateDistributionExchangeRate(exchangeRate.value, security.totp); exchangeRate.value = result.usd_to_cny_rate; app.showSuccess(t('common.saved')) } catch (error) { app.showError(extractApiErrorMessage(error)) } finally { savingRate.value = false } }
