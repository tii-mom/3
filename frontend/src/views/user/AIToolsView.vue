      ...
      <div v-else class="space-y-8">
        <section v-for="group in groupedTools" :key="group.category" class="space-y-4">
          ...
          <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            <article v-for="tool in group.items" :key="tool.id" class="group flex min-h-64 flex-col rounded-3xl border border-gray-200 bg-white p-5 shadow-sm transition hover:-translate-y-1 hover:shadow-xl dark:border-dark-700 dark:bg-dark-900">
              ...
              <p class="mt-3 line-clamp-5 min-h-[7rem] flex-1 text-sm leading-6 text-gray-600 dark:text-gray-300">{{ tool.description || '点开就能用。' }}</p>
