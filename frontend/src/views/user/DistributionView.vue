        ...
        <section v-if="activeTab === 'overview'" class="space-y-6">
          <div class="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
            ...
            <div class="border border-gray-200 p-5 dark:border-dark-700">
              ...
              <p class="mt-1 text-xs leading-5 text-gray-500">{{ t('finance.distribution.rateSnapshot', { rate: exchangeRate }) }}</p>
            </div>
          </div>
...
import { convertToPlatformBalance, createWithdrawal, getDistributionDashboard, getDistributionLedger, getDistributionTree, getPayoutAccount, listWithdrawals, savePayoutAccount, type Commission, type DistributionDashboard, type PayoutAccount, type TeamNode, type Withdrawal } from '@/api/financial'
...
const exchangeRate = computed(() => dashboard.value?.usd_to_cny_rate || '7.15')
const inviteLink = computed(() => inviteDetail.value && typeof window !== 'undefined' ? `${window.location.origin}/register?aff=${encodeURIComponent(inviteDetail.value.aff_code)}` : '')
