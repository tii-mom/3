  ...
  <div class="mx-auto max-w-7xl space-y-6 p-4 sm:p-6 lg:p-8">
    <section class="overflow-hidden rounded-3xl border border-gray-200 bg-white shadow-sm dark:border-dark-700 dark:bg-dark-900">
      <div v-if="banners.length" class="relative">
        ...
        >
        <div class="absolute inset-0 bg-gradient-to-r from-gray-950/80 via-gray-950/45 to-transparent"></div>
        <div class="absolute inset-0 flex items-center p-6 sm:p-10">
          <div class="max-w-xl text-white">
            <p class="mb-3 inline-flex rounded-full bg-white/15 px-3 py-1 text-xs font-medium backdrop-blur">3API 商城</p>
            <h1 class="text-2xl font-bold sm:text-4xl">{{ activeBanner.title }}</h1>
            <p class="mt-3 text-sm leading-6 text-white/80 sm:text-base">{{ activeBanner.subtitle }}</p>
            <button
              v-if="activeBanner.product_id"
              class="mt-5 rounded-full bg-white px-5 py-2.5 text-sm font-semibold text-gray-950 shadow-lg transition hover:-translate-y-0.5"
    ...
    <div v-else-if="products.length === 0" class="rounded-3xl border border-dashed border-gray-300 p-10 text-center dark:border-dark-600">
    ...
    <div v-else class="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
      ...
      >
        <div class="relative h-44 overflow-hidden bg-gray-100 dark:bg-dark-800">
...
const defaultProductImage = 'https://images.unsplash.com/photo-1516321318423-f06f85e504b3?auto=format&fit=crop&w=900&q=80'
