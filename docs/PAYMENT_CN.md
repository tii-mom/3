...
3API 内置支付系统，支持用户自助充值，无需部署独立的支付服务。
...
>
> - **国内渠道 / 人民币结算** — 兼容 ZPay/EasyPay 协议的服务商：可对接支付宝 / 微信支付，资金进入商户账户，具体费率与结算规则以服务商合同为准。
> - **国际渠道 / USDT 或美元结算** — 国际支付聚合服务商：适合需要本地货币支付、美元或 USDT 结算的团队。启用前请自行核实资质、费率、结算周期、提现门槛和合规要求。
>
...
如果你之前使用外部支付系统，现在可以迁移到 3API 内置支付：

### 主要差异

| 对比项 | 外部支付系统 | 内置支付 |
|--------|-----------|---------|
| 部署方式 | 独立服务（Next.js + PostgreSQL） | 内置于 3API，无需额外部署 |
| 支付方式 | EasyPay、支付宝、微信、Stripe | 相同 |
| 配置方式 | 环境变量 + 独立管理后台 | 3API 管理后台内统一配置 |
| 充值对接 | 通过 Admin API 回调 | 内部直接处理，更可靠 |
| 订阅套餐 | 支持 | 暂不支持（计划中） |
| 订单管理 | 独立管理界面 | 集成在 3API 管理后台 |

### 迁移步骤

1. 在 3API 管理后台启用支付并配置服务商（使用相同的支付凭证）
2. 更新 Webhook 回调地址为 3API 的回调地址
3. 确认新订单通过内置支付正常处理
4. 新流程稳定后停用旧外部支付服务

> **注意**：旧外部支付系统中的历史订单数据不会自动迁移。建议保留旧系统一段时间以便查询历史记录。
