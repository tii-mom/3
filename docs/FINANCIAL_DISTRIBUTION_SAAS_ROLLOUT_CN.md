...
- 首次生产预检在访问数据库前拒绝了权限为 `644` 的服务器 `.env`。受保护工作流会先将该文件权限收紧为 `600`，脚本随后再次校验；该步骤不读取或修改文件内容。
- 首次 digest 部署在 Docker 操作前发现服务器部署脚本版本漂移并因非法镜像引用停止。部署工作流改为通过 SSH 标准输入执行同一 `main` 提交中已审查的脚本，不再依赖服务器上的脚本副本。
