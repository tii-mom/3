...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	distributionService := service.NewDistributionService(db, secretEncryptor)
	affiliateService := service.ProvideAffiliateService(affiliateRepository, settingService, apiKeyAuthCacheInvalidator, billingCacheService, distributionService)
	...
	voucherService := service.NewVoucherService(db, settingRepository, totpService, secretEncryptor)
	redeemHandler := handler.NewRedeemHandler(redeemService, voucherService)
	voucherHandler := handler.NewVoucherHandler(voucherService)
	distributionHandler := handler.NewDistributionHandler(distributionService)
	saaSService := service.NewSaaSService(db, settingRepository, apiKeyAuthCacheInvalidator, secretEncryptor)
	partnerHandler := handler.NewPartnerHandler(saaSService)
	...
	shopService.SetSecretEncryptor(secretEncryptor)
	aiToolService := service.NewAIToolService(db)
	paymentService := service.ProvidePaymentService(client, registry, defaultLoadBalancer, redeemService, subscriptionService, paymentConfigService, userRepository, groupRepository, affiliateService, notificationEmailService, distributionService, shopService, apiKeyAuthCacheInvalidator)
	...
	adminDistributionHandler := admin.NewDistributionHandler(distributionService, totpService)
	saaSHandler := admin.NewSaaSHandler(saaSService, totpService, distributionService)
	adminVoucherHandler := admin.NewVoucherHandler(voucherService, totpService)
	adminShopHandler := admin.NewShopHandler(shopService)
	adminAIToolHandler := admin.NewAIToolHandler(aiToolService)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, promptAdminHandler, paymentHandler, affiliateHandler, complianceHandler, auditLogHandler, upstreamBillingProbeService, adminDistributionHandler, saaSHandler, adminVoucherHandler, adminShopHandler, adminAIToolHandler)
	...
	publicShopHandler := handler.NewPublicShopHandler(shopService, paymentConfigService)
	adminShopDeliveryHandler := handler.NewAdminShopDeliveryHandler(shopService)
	aiToolHandler := handler.NewAIToolHandler(aiToolService)
	...
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, voucherHandler, distributionHandler, partnerHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, asyncImageHandler, batchImageHandler, shopHandler, publicShopHandler, adminShopDeliveryHandler, aiToolHandler, idempotencyCoordinator, idempotencyCleanupService)
	...
	engine := server.ProvideRouter(configConfig, handlers, jwtAuthMiddleware, adminAuthMiddleware, apiKeyAuthMiddleware, auditLogMiddleware, stepUpAuthMiddleware, apiKeyService, subscriptionService, opsService, settingService, db, redisClient)
