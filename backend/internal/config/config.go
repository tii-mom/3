...
type GatewayConfig struct {
	...
	// 默认开启；纯文本请求仍只在 Codex 图片能力桥接路径命中时补充工具。
...
func setDefaults() {
	...
	viper.SetDefault("log.service_name", "3api")
	...
	viper.SetDefault("gateway.image_concurrency.enabled", true)
	viper.SetDefault("gateway.image_concurrency.max_concurrent_requests", 10)
	viper.SetDefault("gateway.image_concurrency.overflow_mode", ImageConcurrencyOverflowModeWait)
	viper.SetDefault("gateway.image_concurrency.wait_timeout_seconds", 60)
