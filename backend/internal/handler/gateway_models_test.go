...
func TestGatewayModels_CustomOpenAIModelsListStillAddsImageModelForAllowedGroup(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	var got gatewayModelsResponseForTest
	require.NoError(t, json.Unmarshal(rec.Body.Bytes(), &got))
	require.Equal(t, []string{"gpt-5.5", "gpt-5.4", "gpt-image-2"}, modelIDsForTest(got.Data))
}
