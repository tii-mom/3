...
var ProviderSet = wire.NewSet(
	...
	NewSaaSService,
	ProvideFinancialMaintenanceService,
	NewPromoService,
	NewUsageService,
	NewDashboardService,
...
func ProvideFinancialMaintenanceService(db *sql.DB, vouchers *VoucherService, distribution *DistributionService, saas *SaaSService, redisClient *redis.Client) *FinancialMaintenanceService {
	service := NewFinancialMaintenanceService(db, vouchers, distribution, saas, redisClient)
	service.Start()
	return service
}
...
func ProvideAffiliateService(repo AffiliateRepository, settingService *SettingService, authCacheInvalidator APIKeyAuthCacheInvalidator, billingCacheService *BillingCacheService, distributionService *DistributionService) *AffiliateService {
...
func ProvidePaymentService(entClient *dbent.Client, registry *payment.Registry, loadBalancer payment.LoadBalancer, redeemService *RedeemService, subscriptionSvc *SubscriptionService, configService *PaymentConfigService, userRepo UserRepository, groupRepo GroupRepository, affiliateService *AffiliateService, notificationEmailService *NotificationEmailService, distributionService *DistributionService, shopService *ShopService, authCacheInvalidator APIKeyAuthCacheInvalidator) *PaymentService {
	svc := NewPaymentService(entClient, registry, loadBalancer, redeemService, subscriptionSvc, configService, userRepo, groupRepo, affiliateService)
	svc.SetNotificationEmailService(notificationEmailService)
	svc.SetDistributionService(distributionService)
	svc.SetShopService(shopService)
	svc.SetAuthCacheInvalidator(authCacheInvalidator)
	if shopService != nil {
		shopService.SetPaymentService(svc)
	}
	return svc
}
