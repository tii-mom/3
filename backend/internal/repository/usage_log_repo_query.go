...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	...
	if attemptCount.Valid {
		log.AttemptCount = int(attemptCount.Int64)
	}
	if failoverCount.Valid {
		log.FailoverCount = int(failoverCount.Int64)
	}
	log.AttemptCount, log.FailoverCount = service.NormalizeUsageAttemptCounters(log.AttemptCount, log.FailoverCount)
	if userAgent.Valid {
