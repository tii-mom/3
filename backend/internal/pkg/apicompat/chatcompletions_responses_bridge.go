...
// extractToolOutputMedia rewrites recognized image nodes out of a tool text
// response and re-attaches them as a multimodal user message. Chat-only
// upstreams otherwise receive the image as JSON text, so Codex can neither
// inspect nor render it in the conversation.
