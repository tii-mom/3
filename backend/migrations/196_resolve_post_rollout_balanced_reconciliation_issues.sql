-- Resolve post-rollout reconciliation alerts only when the live invariant
-- already proves that the legacy visible balance and credit-bucket balance
-- are identical.
--
...
SET status = 'RESOLVED',
    ...
    metadata = COALESCE(i.metadata, '{}'::jsonb) || jsonb_build_object(
        'auto_resolved_by', '196_resolve_post_rollout_balanced_reconciliation_issues',
